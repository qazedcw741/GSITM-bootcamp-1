		...
		<div class="row">
			...
			<template>
				...
				<div class="right flex-column flex-100">
                    <div class="right-top">
                        ...
                        <div class="nav-content flex-column flex-gap-10">
                            ...
                            <div class="form-group" style="justify-content: left">
                                <label>납입주기:</label>
                                <select class="form-control" id="pay_ty_cd" v-model="prod.prod_payment_cycle" style="padding-top: 3px;" readonly disabled>
                            ...
                            <div class="form-group" style="justify-content: left">
                                <label>예치금액 (원):</label>
                                <input class="form-control flex-50" type="text" id="circle_acml_amt" v-model="calc.dsgn_sav_paid" style="width: 700px;"/>
                            ...
                            <div class="form-group" style="justify-content: left">
                                <label>예치기간 (개월):</label>
                                <input class="form-control flex-50" type="text" id="goal_prd" v-model="calc.dsgn_sav_target" style="width: 700px;"/>
                            ...
                            <div class="form-group" style="justify-content: left">
                                <label>이자계산법:</label>
								<select class="form-control" id="dsgn_sav_sim_com" v-model="calc.dsgn_sav_sim_com" style="padding-top: 3px;" readonly>
									<option value="1">단리</option>
									<option value="2">복리</option>
								</select>
                            </div>
                        </div>
                    </div>
	                        	...
	                        	<tr>
	                        		<td class="center" style="width: 40%; vertical-align: top;">
	                        			<div class="form-wrapper flex flex-wrap flex-gap-10">
			                                <div class="form-group">
			                                    <label>예치금액:</label>
			                                    <input class="form-control" id="tot_dpst_amt" v-model="calc.dsgn_sav_tot_amt_fmt" disabled />
			                                </div>
			                               <div class="form-group">
			                                    <label>세전이자:</label>
			                                    <input class="form-control" id="tot_dpst_int" v-model="calc.dsgn_sav_tot_int_fmt" disabled />
			                                </div>
			                                <div class="form-group">
			                                    <label>세전수령액:</label>
			                                    <input class="form-control" id="bfo_rcve_amt" v-model="calc.dsgn_sav_bfr_rcve_amt_fmt" disabled />
			                                </div>
			                                <div class="form-group">
			                                    <label>이자과세금:</label>
			                                    <input class="form-control" id="int_tax_amt" v-model="calc.dsgn_sav_int_tax_amt_fmt" disabled />
			                                </div>
			                                <div class="form-group">
			                                    <label>세후수령액:</label>
			                                    <input class="form-control" id="atx_rcve_amt" v-model="calc.dsgn_sav_atx_rcve_amt_fmt" disabled />
			                                </div>
			                            </div>	
			                            ...
			                            <table class="table table-bordered datatable dataTable" id="grid_app">
											<thead>
												<tr class="replace-inputs">
													<th style="width: 18%;" class="center">예치금액</th>
													<!-- <th style="width: 18%;" class="center">회차이자</th> -->
													<th style="width: 18%;" class="center">세전이자</th>
													<th style="width: 23%;" class="center">이자과세금</th>
													<th style="width: 23%;" class="center">세후수령액</th>
												</tr>
											</thead>
											<tbody id="grid_tbody">
												<tr>
													<td class="right" style="text-align : right;">{{calc.dsgn_sav_tot_amt_fmt}}</td>
													<!-- <td class="right" style="text-align : right;">{{calc.dsgn_sav_int}}</td> -->
													<td class="right" style="text-align : right;">{{calc.dsgn_sav_tot_int_fmt}}</td>
													<td class="right" style="text-align : right;">{{calc.dsgn_sav_int_tax_amt_fmt}}</td>
													<td class="right" style="text-align : right;">{{calc.dsgn_sav_atx_rcve_amt_fmt}}</td>
												</tr>
											</tbody>
										</table>
	                        		</td>
                ...
                <div border="1"  class="modal fade" id="printArea">
                    <span style="font-size: 30px; font-weight: bold">[ {{custInfo.cust_name}} ]</span> <span style="font-size: 30px;">고객님!! </span>
                    <span style="color: red; font-size: 30px; font-weight: bold">[ {{prod.prod_name}} ]</span> <span style="font-size: 30px;">가입을 제안 드립니다. </span>
				    <table border="1" style="width: 1000px; height: 700px;">
				       <tr>
							...
							<td class="left" style="width: 35%;">
								...
								<label>예치금액:</label>
								{{calc.dsgn_sav_tot_amt_fmt}} 원<br>
								<label>예치기간:</label>
								{{calc.dsgn_sav_target}} 개월<br>
								<label>적용금리:</label>
								{{prod.pr_max_ir}} %<br>
								<label>이자과세:</label>
				       ...
				       <tr>
							...
							<td class="left">
								...
								{{calc.dsgn_sav_tot_amt_fmt}} 원<br>
								<label>이자:</label>
								{{calc.dsgn_sav_tot_int_fmt}} 원<br>
								<label>총금액:</label>
								{{calc.dsgn_sav_bfr_rcve_amt_fmt}} 원<br>
								<label>세액공제금액:</label>
								{{calc.dsgn_sav_int_tax_amt_fmt}} 원
							</td>
				       </tr>
				    </table>
				</div>
					...
					<table class="table table-bordered datatable dataTable">
						<thead style="position: sticky;top: 0px;">
							<tr>
								...
								<th class="centegetListr" style="width: 20%;">생년월일</th>
...
var vueapp = new Vue({
	...
	data : {
		...
		calc : {
			dsgn_sav_sim_com : "", // 단복리
			dsgn_sav_paid : "", //예치금액
			dsgn_sav_target : "", //예치기간
			dsgn_sav_tot_amt : "", //예치금액합계
			dsgn_sav_tot_int : "", //세전이자
			dsgn_sav_bfr_rcve_amt : "", //세전수령액
			dsgn_sav_int_tax_amt : "", //이자과세금
			dsgn_sav_atx_rcve_amt : "", //세후수령액
			dsgn_sav_int : "", //회차이자
			
			dsgn_sav_tot_amt_fmt : "",
			dsgn_sav_tot_int_fmt : "",
			dsgn_sav_int_tax_amt_fmt : "",
			dsgn_sav_bfr_rcve_amt_fmt : "",
	...
	methods : {
		saveProd : function(){
			var data = {
					dsgn_sav_prod_sn : this.prod.prod_sn, //상품번호
					dsgn_sav_name : this.prod.prod_name, //상품명
					//dsgn_dpst_pay_ty_cd : this.prod.prod_payment_cycle, //납입주기
					dsgn_sav_tax : this.prod.prod_interest_taxation, //이자과세
					dsgn_sav_aply_rate : this.prod.pr_max_ir, //최대금리
					dsgn_sav_date : this.prod.wrt_dt, //작성일자
					dsgn_sav_paid : this.calc.dsgn_sav_paid, //예치금액
					dsgn_sav_target : this.calc.dsgn_sav_target, //예치기간
					dsgn_sav_tot_amt : this.calc.dsgn_sav_tot_amt, //예치금액합계
					dsgn_sav_tot_int  : this.calc.dsgn_sav_tot_int , //세전이자
					dsgn_sav_int_tax_amt  : this.calc.dsgn_sav_int_tax_amt, //이자과세금
					dsgn_sav_atx_rcve_amt : this.calc.dsgn_sav_atx_rcve_amt, //세후수령액
					dsgn_sav_cust_sn : this.custInfo.cust_mbl_telno,//고객코드
					dsgn_sav_sim_com : this.calc.dsgn_sav_sim_com, //단복리
			}
			if(!confirm("저장하시겠습니까?")) return;
			
			cf_ajax("/t1_prod_mng/putProdSav" , data , this.successAlert);
		...
		setCircleAcmlAmt : function(nAmt){
			if(nAmt == 0) {
				this.calc.dsgn_sav_paid = 0;
			}else {
				this.calc.dsgn_sav_paid = Number(this.calc.dsgn_sav_paid) + nAmt*10000;
			}
		},
		setGoalPrd : function(nPrd){
			if(nPrd == 0) {
				this.calc.dsgn_sav_target = 0;
			}else {
				this.calc.dsgn_sav_target = Number(this.calc.dsgn_sav_target) + nPrd;
			}
		},
		popupProd : function(){
			pop_prod.getList();
		...
		prcCalc : function(){
			...
			if(cf_isEmpty(this.prod.prod_name)){
				alert("상품을 선택하세요.");
				return;
			}else if(cf_isEmpty(this.calc.dsgn_sav_paid) || this.calc.dsgn_sav_paid == 0){
				alert("예치금액을 입력하세요.");
				return;
			}else if(cf_isEmpty(this.calc.dsgn_sav_target) || this.calc.dsgn_sav_target == 0){
				alert("예치기간을 입력하세요.");
				return;
			}
			...
			var nPymAmt		= Math.round(this.calc.dsgn_sav_paid); // 예치금액
			//var nRvcy		= Math.round(this.prod.prod_payment_cycle); // 납입주기
			var nPrd		= Math.round(this.calc.dsgn_sav_target); // 예치기간
			...
			if(this.calc.dsgn_sav_sim_com == 1){//단리

				
				var nScInt		= nPymAmt * nApplItr;	// 회차이자
				var nAcmInt		= nScInt * nPrd;	// 세전이자(누적이자)
				
				this.calc.dsgn_sav_tot_amt = (Math.round(nPymAmt));  //예치금액합계
				this.calc.dsgn_sav_int = (Math.round(nScInt)); //회차이자
				this.calc.dsgn_sav_tot_int = (Math.round(nAcmInt)); //세전이자
				this.calc.dsgn_sav_int_tax_amt = (Math.round(nAcmInt * nTax)); //이자과세금
				this.calc.dsgn_sav_bfr_rcve_amt = (Math.round(nPymAmt + nAcmInt)); //세전수령금
				this.calc.dsgn_sav_atx_rcve_amt = (Math.round(this.calc.dsgn_sav_bfr_rcve_amt - this.calc.dsgn_sav_int_tax_amt)); //세후수령금
	        	
				this.calc.dsgn_sav_tot_amt_fmt = this.calc.dsgn_sav_tot_amt.numformat();
				this.calc.dsgn_sav_tot_int_fmt = this.calc.dsgn_sav_tot_int.numformat();
				this.calc.dsgn_sav_int_tax_amt_fmt = this.calc.dsgn_sav_int_tax_amt.numformat();
				this.calc.dsgn_sav_bfr_rcve_amt_fmt = this.calc.dsgn_sav_bfr_rcve_amt.numformat();
				this.calc.dsgn_sav_atx_rcve_amt_fmt = this.calc.dsgn_sav_atx_rcve_amt.numformat();
				this.info.atx_rcve_amt_fmt = this.info.atx_rcve_amt.numformat();
				
				//$("#grid_tbody").html(html);
			}else if(this.calc.dsgn_sav_sim_com == 2){ //복리
				var nScPayAmt	= 0;	// 예치금액
				var nAcmPayAmt	= 0;	// 누적불입금액
				var nScPniAmt	= 0;	// 회차원리금
				var nScInt		= 0;	// 회차이자
				var nAcmInt		= 0;	// 세전이자(누적이자)
				
				var html = '';
				for(var i=1; i<=nPrd; i++) {
					if(i==1){
						nScInt = nPymAmt * nApplItr;
						nScPniAmt = nPymAmt + nScInt;
						nAcmInt =  nScInt;
					}else{
						nScInt = nScPniAmt * nApplItr; //회차이자
						nScPniAmt = nScPniAmt + nScInt; //회차원리금
						nAcmInt = nAcmInt + nScInt; //세전이자(누적이자)
					}
						
					
					
						//nAcmPayAmt = nAcmPayAmt + nScPayAmt;
						//nScInt = nAcmPayAmt * nApplItr;
						//nAcmInt = nAcmInt + nScInt;
						//nScPniAmt = nAcmPayAmt + nAcmInt;
			...
			var chart = bb.generate({
                data: {
                    columns: [
                        ["불입금액합계"	, this.calc.dsgn_sav_tot_amt],
                        ["세전이자"	, this.calc.dsgn_sav_tot_int],
                        ["세전수령액"	, this.calc.dsgn_sav_bfr_rcve_amt],
                        ["이자과세금"	, this.calc.dsgn_sav_int_tax_amt],
                        ["세후수령액"	, this.calc.dsgn_sav_atx_rcve_amt],
                    ],
                    type: "bar",
                    groups: [
                        []
