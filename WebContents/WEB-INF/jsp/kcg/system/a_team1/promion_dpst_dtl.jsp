				...
				<div class="right flex-column flex-100">
                    <div class="right-top">
                        ...
                        <div class="nav-content flex-column flex-gap-10">
                            ...
                            <div class="form-group" style="justify-content: left">
                                <label>불입금액 (원):</label>
                                <input class="form-control flex-50" type="text" id="circle_acml_amt" v-model="calc.dsgn_dpst_amt" style="width: 700px;"/>
                            ...
                            <div class="form-group" style="justify-content: left">
                                <label>목표기간 (개월):</label>
                                <input class="form-control flex-50" type="text" id="goal_prd" v-model="calc.dsgn_dpst_prd" style="width: 700px;"/>
	                        	...
	                        	<tr>
	                        		<td class="center" style="width: 40%; vertical-align: top;">
	                        			<div class="form-wrapper flex flex-wrap flex-gap-10">
			                                <div class="form-group">
			                                    <label>불입금액합계:</label>
			                                    <input class="form-control" id="tot_dpst_amt" v-model="calc.dsgn_dpst_tot_amt_fmt" disabled />
			                                </div>
			                                <div class="form-group">
			                                    <label>세전이자:</label>
			                                    <input class="form-control" id="tot_dpst_int" v-model="calc.dsgn_dpst_tot_int_fmt" disabled />
			                                </div>
			                                <div class="form-group">
			                                    <label>세전수령액:</label>
			                                    <input class="form-control" id="bfo_rcve_amt" v-model="calc.dsgn_dpst_bfr_rcve_amt_fmt" disabled />
			                                </div>
			                                <div class="form-group">
			                                    <label>이자과세금:</label>
			                                    <input class="form-control" id="int_tax_amt" v-model="calc.dsgn_dpst_int_tax_amt_fmt" disabled />
			                                </div>
			                                <div class="form-group">
			                                    <label>세후수령액:</label>
			                                    <input class="form-control" id="atx_rcve_amt" v-model="calc.dsgn_dpst_atx_rcve_amt_fmt" disabled />
			                                </div>
			                            </div>	
				    ...
				    <table border="1" style="width: 1000px; height: 700px;">
				       <tr>
							...
							<td class="left" style="width: 35%;">
								...
								<label>불입금액:</label>
								{{calc.dsgn_dpst_tot_amt_fmt}} 원<br>
								<label>불입기간:</label>
								{{calc.dsgn_dpst_prd}} 개월<br>
								<label>적용금리:</label>
								{{prod.pr_max_ir}} %<br>
								<label>이자과세:</label>
				       ...
				       <tr>
							...
							<td class="left">
								...
								{{calc.dsgn_dpst_tot_amt_fmt}} 원<br>
								<label>이자:</label>
								{{calc.dsgn_dpst_tot_int_fmt}} 원<br>
								<label>총금액:</label>
								{{calc.dsgn_dpst_bfr_rcve_amt_fmt}} 원<br>
								<label>세액공제금액:</label>
								{{calc.dsgn_dpst_int_tax_amt_fmt}} 원
							</td>
				       </tr>
				    </table>
				</div>
...
var vueapp = new Vue({
	...
	data : {
		...
		calc : {
			...
			dsgn_dpst_amt : "", //불입금액
			dsgn_dpst_prd : "", //목표기간
			dsgn_dpst_tot_amt : "", //불입금액합계
			dsgn_dpst_tot_int : "", //세전이자
			dsgn_dpst_bfr_rcve_amt : "", //세전수령액
			dsgn_dpst_int_tax_amt : "", //이자과세금
			dsgn_dpst_atx_rcve_amt : "", //세후수령액
			
			dsgn_dpst_tot_amt_fmt : "",
			dsgn_dpst_tot_int_fmt : "",
			dsgn_dpst_int_tax_amt_fmt : "",
			dsgn_dpst_bfr_rcve_amt_fmt : "",
	...
	methods : {
		saveProd : function(){
			var data = {
					dsgn_dpst_prod_sn : this.prod.prod_sn, //상품번호
					dsgn_dpst_name : this.prod.prod_name, //상품명
					dsgn_dpst_pay_ty_cd : this.prod.prod_payment_cycle, //납입주기
					dsgn_dpst_int_tax_ty_cd : this.prod.prod_interest_taxation, //이자과세
					dsgn_dpst_aply_rate : this.prod.pr_max_ir, //최대금리
					dsgn_dpst_wrt_dt : this.prod.wrt_dt, //작성일자
					dsgn_dpst_amt : this.calc.dsgn_dpst_amt, //불입금액
					dsgn_dpst_prd : this.calc.dsgn_dpst_prd, //목표기간
					dsgn_dpst_tot_amt : this.calc.dsgn_dpst_tot_amt, //불입금액합계
					dsgn_dpst_tot_int : this.calc.dsgn_dpst_tot_int, //세전이자
					dsgn_dpst_int_tax_amt : this.calc.dsgn_dpst_int_tax_amt, //이자과세금
					dsgn_dpst_atx_rcve_amt : this.calc.dsgn_dpst_atx_rcve_amt, //세후수령액
					dsgn_dpst_cust_sn : this.custInfo.cust_mbl_telno, //고객코드
					dsgn_sav_sim_com : this.calc.dsgn_sav_sim_com, //단복리
			}
			if(!confirm("저장하시겠습니까?")) return;
			
			cf_ajax("/t1_prod_mng/putProdDpst" , data , this.successAlert);
		...
		setCircleAcmlAmt : function(nAmt){
			if(nAmt == 0) {
				this.calc.dsgn_dpst_amt = 0;
			}else {
				this.calc.dsgn_dpst_amt = Number(this.calc.dsgn_dpst_amt) + nAmt*10000;
			}
		},
		setGoalPrd : function(nPrd){
			if(nPrd == 0) {
				this.calc.dsgn_dpst_prd = 0;
			}else {
				this.calc.dsgn_dpst_prd = Number(this.calc.dsgn_dpst_prd) + nPrd;
			}
		},
		popupProd : function(){
			pop_prod.getList();
		...
		prcCalc : function(){
			...
			if(cf_isEmpty(this.prod.prod_name)){
				alert("상품을 선택하세요.");
				return;
			}else if(cf_isEmpty(this.calc.dsgn_dpst_amt) || this.calc.dsgn_dpst_amt == 0){
				alert("불입금액을 입력하세요.");
				return;
			}else if(cf_isEmpty(this.calc.dsgn_dpst_prd) || this.calc.dsgn_dpst_prd == 0){
				alert("목표기간을 입력하세요.");
				return;
			}
			...
			var nPrd		= Math.round(this.calc.dsgn_dpst_prd); // 불입기간
			...
			console.log(this.calc.dsgn_sav_sim_com);
			if(this.calc.dsgn_sav_sim_com == 1){ //단리
				...
				for(var i=1; i<=nPrd; i++) {
					...
					if(i==1){
						...
						nAcmPayAmt = nAcmPayAmt + nScPayAmt; //누적불입금액
						nScInt = (nPymAmt + nScPniAmt)*nApplItr; //회차이자
						nAcmInt = nAcmInt + nScInt; //누적이자
						nScPniAmt = nAcmPayAmt + nAcmInt; //회차원리금
						//nScPniAmt = nScPniAmt + nScPayAmt + nScInt;
						//nAcmInt += nScInt;
			...
			this.calc.dsgn_dpst_tot_amt = (Math.round(nAcmPayAmt)); //불입총금액
			this.calc.dsgn_dpst_tot_int = (Math.round(nAcmInt)); //세전이자
			this.calc.dsgn_dpst_int_tax_amt = (Math.round(nAcmInt * nTax)); //이자과세금
			this.calc.dsgn_dpst_bfr_rcve_amt = (Math.round(nScPniAmt)); //세전수령금
			this.calc.dsgn_dpst_atx_rcve_amt = (Math.round(nScPniAmt - this.calc.dsgn_dpst_int_tax_amt)); //세후수령금
        	
			this.calc.dsgn_dpst_tot_amt_fmt = this.calc.dsgn_dpst_tot_amt.numformat();
			this.calc.dsgn_dpst_tot_int_fmt = this.calc.dsgn_dpst_tot_int.numformat();
			this.calc.dsgn_dpst_int_tax_amt_fmt = this.calc.dsgn_dpst_int_tax_amt.numformat();
			this.calc.dsgn_dpst_bfr_rcve_amt_fmt = this.calc.dsgn_dpst_bfr_rcve_amt.numformat();
			this.calc.dsgn_dpst_atx_rcve_amt_fmt = this.calc.dsgn_dpst_atx_rcve_amt.numformat();
			...
			var chart = bb.generate({
                data: {
                    columns: [
                        ["불입금액합계"	, this.calc.dsgn_dpst_tot_amt],
                        ["세전이자"	, this.calc.dsgn_dpst_tot_int],
                        ["세전수령액"	, this.calc.dsgn_dpst_bfr_rcve_amt],
                        ["이자과세금"	, this.calc.dsgn_dpst_int_tax_amt],
                        ["세후수령액"	, this.calc.dsgn_dpst_atx_rcve_amt],
                    ],
                    type: "bar",
                    groups: [
                        []
...
var prod = new Vue ({
	...
	methods : {
		getProdOne : function(){
			...
			cf_ajax("/t1_prod_mng/getProdOne"  , prod ,this.getProd);
		},
